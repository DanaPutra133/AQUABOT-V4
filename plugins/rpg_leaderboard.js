let handler = async (m, { conn, args, participants }) => {
  ...
  function getRank(array, sender) {
      let index = array.findIndex(user => user.jid === sender);
      return index >= 0 ? index + 1 : "Tidak terdaftar";
  }
...
Kamu: *${getRank(sortedExp, m.sender)}* dari *${sortedExp.length}*

${sortedExp.slice(0, len).map(({ jid, exp }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${exp} Exp*`).join('\n')}

• *Limit Leaderboard Top ${len}* •
Kamu: *${getRank(sortedLim, m.sender)}* dari *${sortedLim.length}*
...
Kamu: *${getRank(sortedLevel, m.sender)}* dari *${sortedLevel.length}*
...
Kamu: *${getRank(sortedMoney, m.sender)}* dari *${sortedMoney.length}*
...
Kamu: *${getRank(sortedBank, m.sender)}* dari *${sortedBank.length}*

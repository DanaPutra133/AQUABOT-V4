...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    try {
        ...
        if (!kategori) {
            throw `Silakan pilih kategori gambar yang tersedia.\n\n*Kategori Tersedia:*\n- ${kategoriList.join('\n- ')}\n\n*Contoh Penggunaan:*\n${usedPrefix + command} wallpaper`;
        }
        
        if (!kategoriList.includes(kategori)) {
            throw `Kategori "${kategori}" tidak ditemukan.\n\n*Kategori yang Tersedia:*\n- ${kategoriList.join('\n- ')}`;
        }

        await m.reply(`⏳ Sedang mencari gambar random dari kategori *${kategori}*...`);
        ...
        if (jsonData && jsonData.status && jsonData.urls && jsonData.urls.length > 0) {
            ...
            await conn.sendFile(m.chat, imageUrl, 'random.jpg', `Ini gambar random *${kategori}* untukmu!`, m);
        } else {

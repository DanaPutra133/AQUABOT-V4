let handler = async (m, { conn, command, args }) => {
    try { 
        ...
        if (!Array.isArray(tasksList)) throw new Error('Format data dari API tidak valid (harusnya array).');
        ...
        if (!kategori) {
            let msg = `*Pilih kategori rekap yang ingin dilihat:*\n\n`;
            msg += `• .${command} semua\n`;
        ...
        if (kategori === 'semua') {
            const className = resJson.classInfo?.nama || '3KA14';
